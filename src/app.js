...
app.get("/buyer-dashboard", requireLogin, (req, res) => {
  ...
  data.exec(function (err, items) {
    ...
    res.render("buyer-dash", { items: items });
  });
});
...
app.get("/seller-dashboard", requireLogin, (req, res) => {
  ...
  data.exec(function (err, items) {
    if (err) return handleError(err);
    res.render("seller-dash", { items: items });
  });
});
...
app.get("/useller-dashboard", requireLogin, (req, res) => {
  ...
  data.exec(function (err, items) {
    if (err) return handleError(err);
    res.render("useller-dash", { items: items });
  });
});
    ...
    if (req.session.seller === true) {
      ...
      if (seller) {
        ...
        const sms = await fast2sms.sendMessage({
          ...
          message: `OTP (One Time Password) For Logging in to pocketlandealer is ${otp}. Do not share it with anyone`,
          numbers: [req.body.mobNo],
        });
    ...
    if (req.session.seller === true) {
      ...
      if (ruser) {
        ...
        if (npass === cnpass) {
          ...
          await sellerRegister.updateOne(
            { password: ruser.password },
            { $set: { password: hashpassword } }
          );
          req.flash(
            "success",
            "Password Reset Successfull !!! Proceed To Login"
          );
          res.redirect("/login");
        } else {
          req.flash(
  ...
  try {
    ...
    transporter.sendMail(mailoptions, async (err, info) => {
      if (err) {
        ...
        req.flash(
          "success",
          "An Email has been sent to the Owner for your Request"
        );
        res.redirect("/buyer-dashboard");
      }
    });

    const message = await fast2sms.sendMessage({
      authorization: process.env.FAST2SMS_API_KEY,
      message: `A buyer is interested in your plot, so check your email for further details`,
      numbers: [seller.userdata.mobNo],
    });
  } catch (error) {
  ...
  try {
    if (req.session.passport) {
      ...
      if (user) {
        ...
        const { firstName, middleName, lastName, mobile, email, price } =
          req.body;

        await Seller.updateOne(
          { _id: userid },
          { $set: { "userdata.firstName": firstName } }
        );
        await Seller.updateOne(
          { _id: userid },
          { $set: { "userdata.middleName": middleName } }
        );
        await Seller.updateOne(
          { _id: userid },
          { $set: { "userdata.lastName": lastName } }
        );
        await Seller.updateOne(
          { _id: userid },
          { $set: { "userdata.mobNo": mobile } }
        );
        await Seller.updateOne(
          { _id: userid },
